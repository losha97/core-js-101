...
function getPolynom(...rest) {
  ...
  const arr = zeros.splice(rest.length, zeros.length).concat(rest);
...
function memoize(func) {
  ...
  return (argument) => {
    if (memory.has(argument)) {
      return memory.get(argument);
    }
    const result = func(argument);
    memory.set(argument, result);
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...x) => {
    const res = JSON.stringify(Array.from(x.values())).slice(1, -1);
    logFunc(`${func.name}(${res}) starts\n${func.name}(${res}) ends`);
    return func(x);
  };
}
